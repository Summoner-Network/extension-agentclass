...
# Collect requirement files: .\requirements.txt agents\*\requirements.txt and analytics\*\requirements.txt
$files = @()

if (Test-Path -LiteralPath (Join-Path $PSScriptRoot "tests")) {
  $files += Get-ChildItem -LiteralPath (Join-Path $PSScriptRoot "tests") -Recurse -Filter "requirements.txt" -ErrorAction SilentlyContinue |
...
if (-not $files -or $files.Count -eq 0) {
  Write-Host -ForegroundColor Yellow "No requirements.txt files found under agents\*, tests\*, or next to this script."
  return
}
